...
class HaploOmniMultiModalNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.use_SSG:
            shift_C, scale_C, gate_C = self.SSG_C.unsqueeze(1).repeat(1, bs, 1).chunk(3, dim=0)   # 3 x [bs, dim]
            shift_L, scale_L, gate_L = self.SSG_L.unsqueeze(1).repeat(1, bs, 1).chunk(3, dim=0)   # 3 x [bs, dim]
